...
c-\sum _{i=1}^{n-1} x_i \right) ^2 }_{x_n^2} \\
...
\right) .
\end{equation}
It follows immediately that $\sum _j e_{ij}=x_i+\frac{1-c}{n}$, which is exactly \eqref{eq:sumeij}.
Inserting this into equation \eqref{eq:modumatrix} gives the same expression for modularity \eqref{eq:Qndim} as for the general case:
\begin{equation*}
Q=c-\sum _i x_i^2 - \frac{1-c^2}{n} = \sum _i x_i -\sum _i x_i^2 - \frac{1-(\sum _i x_i)^2}{n}.
\end{equation*}
