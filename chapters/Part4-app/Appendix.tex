...
We generated networks with varying reciprocity using algorithm \ref{alg:reciprocity}.
It uses the following external functions:
Function \verb"random" returns a random number in the interval $[0,1]$ and \verb"choice" returns one input object randomly.
%
\begin{algorithm}[htbp]
\SetKwFunction{random}{random}
\SetKwFunction{DirectedGraph}{DirectedGraph}
\SetKwFunction{choice}{choice}%\SetAlgoLined  \KwData{$N$, $p$, $p_\mathrm{rev}$}  \KwResult{$G$, $\rho $}
\BlankLine
\tcp{Generate undirected \ER graph}
initialize empty graph $H$ with $N$ nodes\;
  \For{all node pairs $(u,v)$ in $H$}{    r = \random{}\;    \If{$p<r$}{      add edge $(u,v)$ to $H$\;      }
}
\BlankLine
\tcp{Replace all undirected edges by two bidirectional directed edges}
initialize empty directed graph $G$ with $N$ nodes\;
\For{edge $(u,v)$ in $H$}{add edges $(u,v)$ and $(v,u)$ to $G$\;}
\BlankLine
\tcp{Replace bidirectional edges with probability $p_\mathrm{rev}$}
\For{edge $(u,v)$ in $H$}{
r = \random{} \;
\If{$p_\mathrm{rev}<r$}{
remove edge \choice{$(u,v),(v,u)$} from $G$\;
}
}
\BlankLine
Compute reciprocity $\rho$ of $G$ according equation \eqref{eq:link_reciprocity}\;
%
%\caption{Generate networks of fixed reciprocity}
\label{alg:reciprocity}\end{algorithm}

Networks of varying modularity were computed as follows:
