...
Therefore, we can assess a period of 1 year sufficient to provide stationarity of the system, i.e. the time span, after which the system evolves periodically.

\subsection{Node rankings}
A ranking of nodes is of major importance for epidemiology.
...
\caption{Temporal variation in the range $r_u(k,t_0)$ of an exemplary node $u$ in the network over one year.
Although the range remains rather constant for most infection times, it vanishes for certain periods.
Infectious period is fixed, $k=24$ days.
}
\label{fig:range_exemplary}
\end{SCfigure}

Figure \ref{fig:range_exemplary} shows the range of an exemplary node in the network for different times of infection.
