...
\usepackage{graphicx,color,scrpage2,natbib}
...
The clustering coefficient plays an essential role in the small-world model of networks (\cite{Watts:2006}, section \ref{sec:network_models}) and has been found to be an important property not only in social networks \cite{Holland1971}, but in many real-world networks \cite{Newman2003}.
...
It is an important building block of the Watts-Strogatz network model (\cite{Watts:2006}, section \ref{sec:watts_strogatz_model}).
...
A \emph{random network} is generated by generating a set of $N$ nodes and connect each of the $N(N-1)$ possible node pairs\footnote{We focus on directed networks here. In the undirected case, there are $\frac{1}{2}N(N-1)$ possible node pairs.} with a certain probability $p$.
Networks generated this way are often called $G_{N,p}$ networks or $G_{N,p}$ ensembles\footnote{An equivalent approach is to consider a fixed number of edges $m$ instead, yielding a $G_{N,m}$ graph.}.
...
Apparently, the expected number of edges in the network is $\mean{E}=pN(N-1)$, if $p$ is the edge occupation probability.
In addition, the average degree of a random network of $N$ nodes is $\mean{k}=pN$.

An interesting feature of random graphs is that for different edge occupation probabilities they show different phases.
...
\caption{Emergence of the largest connected component for an ensemble of \ER graphs.
The critical value $p_c = 0.001$ corresponds to an average degree of 1.
...
\subsection{Watt-Strogatz model}\label{sec:watts_strogatz_model}
