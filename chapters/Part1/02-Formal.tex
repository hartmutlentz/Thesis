...
This assumption is reasonable, if no further information about the contact structure within a population is available.
...
However, the random mixing assumption does not allow for non heterogeneous mixing, since each individual is considered equal.
...
Thus, a network clustering coefficient can also be defined by the average $\mean{c_i}$, which gives slightly different values than \eqref{eq:clustering_coefficient} and should not be mixed up with the latter.
...
Accessibility measures the ability of each node to reach destinations, which is of particular importance for transportation systems \citep{Garrison:1960up,mackewitz}.
