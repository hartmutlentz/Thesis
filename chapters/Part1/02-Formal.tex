...
%
%\paragraph{Research field: Epidemiology\color{Cayenne}{.}}
...
In the simplest case, the infection status of each individual is either susceptible or infected and there are no births and deaths in the population.
...
In contrast to the infection dynamics introduced in the previous section, many epidemics allow for an immunization of the individuals.
...
Vaccination basically decreases the size of the initial susceptible population $S_*$ .
A reduction of the infection rate can be archieved by increasing hygiene standards or appropriate behavior, say wearing warm clothes in winter time to avoid common cold.
...
Infection rates themselves can cause complex infection dynamics.
...
This assumption seems reasonable, if no further information about the contact structure within a population is available.
...
However, the random mixing assumption does not allow for non heterogenous mixing, since each individual is considered equal.
Equality of individuals is not a reasonable assumption for many epidemic substrates.
Examples of epidemic substrates are contact structures of humans, livestock trade or links between computers.
Apparently, not all pairs of individuals are connected in these systems.
...
This meta information can be their importance, capacity, number of transported items or the geographical distance between the nodes they connect.
%
\begin{SCfigure}%[h]
%\begin{center}
...
We denote the number of nodes in the network by $\abs{V}=N$ and the number of edges by $m=\abs{E}$.
...
Given a network, an important question is, if some nodes are more important than other nodes.
...
As a matter of fact, the distribution of a networks degrees has been proven usefuls for the classification into different network types.
...
It should be noted that there is also a node clustering coefficient defined by $c_i=\sum _{jl} a_{ij}a_{jl}a_{li}/(k_i (k_i -1))$ \citep{Watts:1998,dynamical_processes}.
Thus, a network clustering coefficient can also be defined by the average $\mean{c_i}$, which gives slightly different values as \eqref{eq:clustering_coefficient} and should not be mixed up with the latter.
...
If we directly connect each node of a network with all other nodes it is connected to by a path of whatever length, we obtain the \emph{accessibility} of the network.
Accessibility measures the ability of each node to reach destinations, which is in particular important for transportation systems \citep{Garrison:1960up,mackewitz}.
...
We choose the mean degree is a convenient parameter for the analysis of random graphs, since it can be used to replace the explicit system size.
...
The giant component contains almost all nodes of the network.
...
In their paper about properties of small-world networks \citep{Barrat:2000fj}, the authors showed that the degree distribution converges to a Poisson distribution in the limit $p\rightarrow 1$ and found an analytical approximation for the clustering coefficient for different values of $p$.
...
In order to obtain a rate-equation for the total number of infected in a homogenous network, we replace the local degree $k$ by the mean degree $\mean{k}$ and obtain
...
This shows that the threshold behavior seen in Section~\ref{sec:sir_model} for homogeneously mixed populations remains unchanged for homogenous networks.
...
A \emph{meta-population} is a set of subpopulations which are connected by migration processes \citep{Grenfell:1997ts,Hanski:1998,dynamical_processes}.
Recent works have made use of meta-population approaches to model large scale disease outbreaks \citep{VittoriaColizza02142006}, such as influenza \citep{DuyguBalcan12222009} and SARS \citep{Hufnagel:2004p27}.
