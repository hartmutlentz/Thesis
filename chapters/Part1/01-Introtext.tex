...
Early attempts go back to the 18th century; in his review about the mathematics of infectious diseases, \citeauthor{Hethcote:2000} reports a model for smallpox was already formulated in 1760 by D. Bernoulli (see \citet{Hethcote:2000} and references therein).

In the early 20th century, mathematical models for epidemics were developed: a discrete time model in 1906 \citep{Hamer} and a differential equation model in 1911 \citep{Ross}.
...
Besides infectious diseases of humans, livestock epidemics are a major economic issue in agriculture.
...
Due to legislation introduced 2001 after the BSE crisis, large amounts of data of livestock movements have been collected in Europe.
...
Network analysis has become an essential element of epidemiology, where networks model interactions between the individuals of a population.
...
\citeauthor{euler:1736} solved the so-called seven bridges of Kšnigsberg problem by showing that there is no closed path using all edges of a network exactly once, if more than two nodes have an odd number of neighbors \citep{euler:1736}.
...
Beyond the tools and methods of graph theory, the origins of modern network science go back to \emph{sociology}.
...
The path structure between the nodes of a network is closely related to its percolation properties.
...
As a concept similar to components, densely connected subgraphs -- so called modules -- were introduced by \citet{Newman06062006}.
Modules allow for a statistically small number of paths between each other.
These structures have been observed in the livestock network analyzed in this thesis \citep{Lentz:2011} and in other networks \citep{Clauset_greedy,Fortunanto2010}.

The impact of modular structure on disease spreading has been studied for social networks by \citet{Salathe:2010p6333}.
However, for the case livestock trade networks, the impact of a modular structure has not been analyzed systematically yet.
...
In contrast to static network analysis, a number of problems arise from the importance of causality in temporal network analysis \citep{Casteights_review,Nicosia:2012hz}.
...
On the local mobility level, a network of bike sharing locations has been investigated by \citet{Vogel:2011bb}, where the authors found different node classes according to a similarity of temporal degree patterns.
Temporal contact patterns in form of a growing network of sexual contacts were analyzed by \citet{Rocha_pnas,Rocha_plosbc}.
\citeauthor{Rocha_pnas} found a preferential attachment rule for the growing web community.
...
Network snapshots have also been used in order to generalized the concept of small-world networks in \citep{Tang:2010dn}, where clustering is measured in terms of the persistence of links over time.
Besides the temporal network model introduced by \citet{Tang:2010dn}, random walk models can be used in order generate synthetic temporal networks reproducing the bursty behavior of any real-world dataset \citep{Barrat:2013da}.
...
What is still missing concerning temporal network analysis is a closed mathematical formalism preserving the causality of paths in temporal networks. 
...
%
In \textbf{Chapter~\ref{chap:static}}, for the first time we systematically analyze the trade of livestock in Germany as a static network.
Hereby, we identify its path structure as a crucial epidemiological factor in Section~\ref{sec:network_analysis}.
The spreading potential of the observed path structure is analyzed numerically in Section~\ref{sec:PRE}.
%
\textbf{Chapter~\ref{sec:temporal_networks}} is devoted to considering the full temporal information of the livestock trade network.
...
Finally, we introduce the unfolding accessibility method as a new approach to measure the causal path structure in temporal networks in Section~\ref{sec:PRL}.
Finally, we use accessibility in order to quantify the goodness of an aggregated representation of a temporal network in Section~\ref{sec:causal_fidelity}.
